...
local function has_ansi()
    local is_windows =
        type(package) == "table" and type(package.config) == "string" and package.config:sub(1, 1) == "\\"
    if is_windows then
...
local function append_perfdata(s, dedicated_page)
    ...
    local target_fps =
        ds and mp.get_property_number("display-fps", 0) or mp.get_property_number(compat("container-fps"), 0)
